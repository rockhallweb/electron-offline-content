...
The release workflow uses `id-token: write` only on the job that calls npm Trusted Publishing. That staging job runs on a GitHub-hosted runner because npm Trusted Publishing does not currently support self-hosted runners. The workflow does not use an `NPM_TOKEN` secret and should only be allowed to run `npm stage publish`, not `npm publish`. See [`docs/release.md`](release.md) for the release process and npm setup.
